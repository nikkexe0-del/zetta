...
import com.deniscerri.ytdl.util.BatteryOptimisation
...
class MainActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        askPermissions()
        // zetta: the settings entry for this was removed, so ask once here instead
        BatteryOptimisation.requestOnceIfNeeded(this)
        ...
        navigationView?.apply {
            setupWithNavController(navController)
            //settings button
            menu.findItem(R.id.activitySettings).setOnMenuItemClickListener {
                val intent = Intent(context, SettingsActivity::class.java)
                startActivity(intent)
                true
            }
    ...
    private fun askAutoUpdatePreferences() {
        if (preferences.getBoolean("asked_auto_update_preferences", false)) {
            // zetta suppresses the first-run update dialog and pins these on in ManagedDefaults,
            // so this early-return is now the ONLY path taken. The dialog's OK handler used to be
            // the only place UpdateCheckWorker was ever scheduled -- without re-scheduling here,
            // skipping the dialog would silently leave the periodic app-update check unregistered.
            // enqueueUniquePeriodicWork(UPDATE) makes this idempotent, so calling it every launch
            // is safe.
            if (BuildConfig.FLAVOR == "github" && preferences.getBoolean("update_app", true)) {
