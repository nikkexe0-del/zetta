...
import com.deniscerri.ytdl.ui.download.QuickDownloadController
import com.deniscerri.ytdl.ui.bookmarks.HomeStripController
...
class HomeFragment : Fragment(), HomeAdapter.OnItemClickListener, SearchSuggestionsAdapter.OnItemClickListener, OnClickListener {
    ...
    private var homeStripController: HomeStripController? = null
    private var quickDownloadController: QuickDownloadController? = null
    private var quickDownloadCard: View? = null
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // zetta: bookmark folders + quick quality picker. Self contained, nothing here
        // touches search or the download pipeline.
        homeStripController = HomeStripController(requireContext(), view, findNavController()).apply {
            attach()
        }

        // zetta: quick download flow card below the folders
        view.findViewById<View>(R.id.home_bookmarks_strip)?.parent?.let {
            if (it is ViewGroup) {
                val downloadCard = LayoutInflater.from(context)
                    .inflate(R.layout.download_quick_flow, it, false)
                it.addView(downloadCard, it.indexOfChild(recyclerView))
                quickDownloadCard = downloadCard
                quickDownloadController = QuickDownloadController(
                    requireContext(),
                    downloadCard,
                    resultViewModel,
                    downloadViewModel,
        ...
        lifecycleScope.launch {
            launch{
                resultViewModel.uiState.collectLatest { res ->
                    ...
                    homeStripController?.setVisible(totalCount == 0 && !res.processing)
                    // hide quick download too while results are showing
                    quickDownloadCard?.isVisible = totalCount == 0 && !res.processing
                    if (res.processing){
...
//            searchSuggestionsLinearLayout!!.isVisible = suggestions.isNotEmpty()
    ...
    override fun onDestroyView() {
        // These hold views from the destroyed hierarchy; the fragment instance outlives it.
        quickDownloadCard = null
        quickDownloadController = null
        homeStripController = null
        super.onDestroyView()
    }
}