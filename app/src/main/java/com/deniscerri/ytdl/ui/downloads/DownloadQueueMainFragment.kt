...
class DownloadQueueMainFragment : Fragment(){
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // zetta: exactly five pills -- Downloaded, Running, In Queue, Errored, Cancelled.
        // Scheduled and Saved were dropped from the pill row to keep it simple; their fragments
        // still exist and every index-based reference below is null-guarded, so nothing breaks.
        ...
        TabLayoutMediator(tabLayout, viewPager2) { tab, position ->
            when (position) {
                0 -> tab.text = getString(R.string.downloads)
                1 -> tab.text = getString(R.string.running)
                2 -> tab.text = getString(R.string.in_queue)
                3 -> tab.text = getString(R.string.errored)
                4 -> tab.text = getString(R.string.cancelled)
            }
        }.attach()
    ...
    fun scrollToActive(){
        // running is tab 1 now that Downloaded sits in front of it
