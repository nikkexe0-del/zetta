...
class NotificationUtil(var context: Context) {
    ...
    fun createDownloadFinished(
        ...
        val sizeText = if (filesize > 1) FileUtil.convertFileSize(filesize) else ""
        val sizeSuffix = if (sizeText.isNotEmpty()) " • $sizeText" else ""
        val safeTitle = clip(displayTitle(title), 40)
        val contentText = StringBuilder("$safeTitle$sizeSuffix")

        val bitmap = iconType.toBitmap(context)
        notificationBuilder
            .setContentTitle(clip("${res.getString(R.string.downloaded)} ${displayTitle(title)}", 44))
            .setSubText(if (sizeText.isNotEmpty()) sizeText else null)
            .setSmallIcon(R.drawable.ic_launcher_foreground_large)
            .setLargeIcon(bitmap)
            .setGroup(DOWNLOAD_FINISHED_NOTIFICATION_ID.toString())
            .setContentText(clip("$safeTitle$sizeSuffix", 60))
    ...
    @SuppressLint("MissingPermission")
    /**
     * Renders a 10 cell text progress bar, e.g. 82% -> ████████░░
     */
    private fun progressBar(progress: Int) : String {
        val clamped = progress.coerceIn(0, 100)
        val filled = Math.round(clamped / 10f)
        return "█".repeat(filled) + "░".repeat(10 - filled)
    }

    /**
     * Notification title/text fields are a single line in the collapsed shade. Video titles are
     * routinely far longer than that, and letting the system hard-truncate mid-word looks broken,
     * so clip on a word boundary and add an ellipsis ourselves.
     */
    ...
    /** One line, always fits: "Downloading ████████░░ 82%" */
    private fun progressLine(progress: Int) : String =
        "${resources.getString(R.string.downloading)} ${progressBar(progress)} $progress%"

    /**
     * Playful status words shown while a shared link is still being resolved, before yt-dlp has
     * handed back a title. Cycles on each notification update so the shade shows life instead of
     * a frozen raw URL.
     */
    private val WARMUP_STEP_MS = 750L

    private val warmupWords = listOf(
        "Sniffing", "Fathoming", "Untangling", "Rummaging",
        "Divining", "Wrangling", "Conjuring", "Noodling"
    )

    fun warmupWord(step: Int) : String =
        warmupWords[Math.floorMod(step, warmupWords.size)] + "…"

    /**
     * A shared link starts life as a bare URL. Showing that in the notification is noise, so fall
     * back to the host name until the real title arrives.
     */
    /**
     * True when a "title" is really just the raw shared link, which is what a download item carries
     * until yt-dlp has resolved metadata.
     */
    fun looksLikeUrl(title: String?) : Boolean =
        title.isNullOrBlank() || title.startsWith("http://") || title.startsWith("https://")

    /** Never show a raw URL in the shade: fall back to the host name. */
    fun displayTitle(title: String?) : String =
        if (looksLikeUrl(title)) placeholderTitleFor(title.orEmpty()) else title!!

    fun placeholderTitleFor(url: String) : String {
        val host = runCatching {
            java.net.URI(url).host.orEmpty().removePrefix("www.")
        }.getOrDefault("")
        return if (host.isBlank()) resources.getString(R.string.downloading)
               else host.substringBefore('.').replaceFirstChar { it.uppercase() }
    }

    /**
     * Posted the moment a shared link is queued, so the user gets immediate feedback without the
     * app ever opening. Uses the same notification id as the running download, so the real
     * progress notification simply takes its place.
     */
    @SuppressLint("MissingPermission")
    fun createSharedLinkQueuedNotification(id: Int, title: String?, url: String = "", step: Int = 0) {
        runCatching {
            val shown = if (looksLikeUrl(title)) placeholderTitleFor(if (url.isNotBlank()) url else title.orEmpty())
                        else title!!

            val notificationBuilder = getBuilder(DOWNLOAD_SERVICE_CHANNEL_ID)
            notificationBuilder
                .setContentTitle(clip(shown, 40))
                .setContentText(warmupWord(step))
                .setSmallIcon(R.drawable.ic_launcher_foreground_large)
                .setOngoing(true)
    ...
    fun updateDownloadNotification(
        ...
        // Collapsed shade gets exactly one short line; the detail lives in the expanded view.
        val safeTitle = clip(displayTitle(title), 40)
        // Before yt-dlp reports any progress we are still resolving the link, so keep the playful
        // status word running instead of showing a 0% bar against a URL.
        val stillResolving = progress <= 0 && looksLikeUrl(title)
        val collapsedLine = if (stillResolving) {
            warmupWord(((System.currentTimeMillis() / WARMUP_STEP_MS).toInt()))
        } else {
            progressLine(progress)
        }
        val itemsLeft = if (queue > 1) "${queue - 1} ${resources.getString(R.string.items_left)}" else ""
        val cleanedDesc = desc.replace("\\[.*?\\] ".toRegex(), "").trim()

        val expandedText = buildString {
            append(collapsedLine)
            if (itemsLeft.isNotEmpty()) append("\n").append(itemsLeft)
            if (cleanedDesc.isNotEmpty()) append("\n").append(clip(cleanedDesc, 120))
        }
        ...
        try {
            if (canPostPromotedNotifications) {
                ...
                val builder = Notification.Builder(context, channel)
                    ...
                    .setSubText(if (itemsLeft.isEmpty()) null else itemsLeft)
