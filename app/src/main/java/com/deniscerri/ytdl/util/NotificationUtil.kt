...
class NotificationUtil(var context: Context) {
    ...
    private fun buildTextProgressBar(progress: Int, length: Int = 10): String {
        val clamped = progress.coerceIn(0, 100)
        val filled = (clamped * length) / 100
    ...
    fun createDownloadFinished(
        ...
        val totalSize = filepath?.sumOf { runCatching { File(it).length() }.getOrDefault(0L) } ?: 0L
        val sizeSuffix = if (totalSize > 0) " (${FileUtil.convertFileSize(totalSize)})" else ""

        val bitmap = iconType.toBitmap(context)
        notificationBuilder
            .setContentTitle("${res.getString(R.string.downloaded)} $title$sizeSuffix")
            .setSmallIcon(R.drawable.ic_launcher_foreground_large)
            .setLargeIcon(bitmap)
            .setGroup(DOWNLOAD_FINISHED_NOTIFICATION_ID.toString())
            .setContentText("$title$sizeSuffix")
        ...
        try {
            if (canPostPromotedNotifications) {
                ...
                val builder = Notification.Builder(context, channel)
                    .setSmallIcon(R.drawable.ic_launcher_foreground_large)
                    .setContentTitle(title)
                    .setContentText("${resources.getString(R.string.downloading)}... ${buildTextProgressBar(progress)} $progress%")
                ...
                val notificationBuilder = getBuilder(channel)
                val progressLine = "${resources.getString(R.string.downloading)}... ${buildTextProgressBar(progress)} $progress%"
                notificationBuilder.setProgress(100, progress, (progress == 0 || progress == 100))
                    .setContentTitle(title)
                    .setContentText(progressLine)
                    .setStyle(NotificationCompat.BigTextStyle().bigText("$progressLine\n$contentText"))
                    .setGroup(DOWNLOAD_RUNNING_NOTIFICATION_ID.toString())
                    .clearActions()
