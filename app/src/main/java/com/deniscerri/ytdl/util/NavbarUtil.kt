...
object NavbarUtil {
    ...
    // must stay in sync with res/menu/bottom_nav_menu.xml, every id in that menu needs an entry
    private var navItems = mapOf(
        "Home" to R.id.homeFragment,
        "Downloads" to R.id.downloadQueueMainFragment,
        "Analytics" to R.id.analyticsFragment,
        "Settings" to R.id.moreFragment
    )
    ...
    fun setStartFragment(itemId: Int) {
        val key = navItems.entries.firstOrNull { it.value == itemId }?.key ?: return
        settings.edit().putString("start_destination", key).apply()
    }
    ...
    private fun getNavBarPrefs(): List<String> {
        return settings
            .getString("navigation_bar", "0,1,2,3")!!
            .split(",")
    }
